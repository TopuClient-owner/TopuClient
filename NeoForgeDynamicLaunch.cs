...
namespace TopuLauncher
{
    public partial class MainWindow
    {
        ...
        {
            EventManager.RegisterClassHandler(typeof(Button), Button.PreviewMouseLeftButtonDownEvent, new MouseButtonEventHandler(DynamicNeoForgeLaunchPreview), true);
            return true;
        }

        private static void DynamicNeoForgeLaunchPreview(object sender, MouseButtonEventArgs e)
        {
            if (sender is not Button button || button.DataContext is MainWindow) return;
            MainWindow? window = Window.GetWindow(button) as MainWindow;
            if (window == null || window.LaunchBtn != button) return;
            RuntimeProfileSettings profile = window.GetRuntimeProfile();
            if (!profile.Loader.Equals("NeoForge", StringComparison.OrdinalIgnoreCase)) return;
            e.Handled = true;
        ...
        private async Task LaunchDynamicNeoForgeAsync(RuntimeProfileSettings profile)
        {
            ...
            {
                try { if (!_minecraftProcess.HasExited) { MessageBox.Show("Minecraft is already running.", "Topu Client", MessageBoxButton.OK, MessageBoxImage.Information); return; } }
                catch { }
            ...
            try
            {
                ...
                if (_session == null) throw new InvalidOperationException("Could not create a Minecraft session.");
                ...
                if (process == null) throw new InvalidOperationException("CmlLib returned a null Minecraft process.");
                ...
                if (!process.Start()) throw new InvalidOperationException("Windows failed to start Minecraft.");
